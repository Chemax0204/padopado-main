...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			default:
				defaultCamZoom = 0.8;
				curStage = 'home';
				var bg:BGSprite = new BGSprite('stageStuff/BackgroundPADO', -600, -150, ['BackPado'], true);
				add(bg);

				var characters:BGSprite = new BGSprite('stageStuff/Background_2', -250, 350, ['back'], true);
				characters.scale.set(0.8, 0.8);
				characters.updateHitbox();
				add(characters);

				var characters:BGSprite = new BGSprite('stageStuff/Background_3', -70, 0, ['Back 2'], true);
				characters.scale.set(0.8, 0.8);
				characters.updateHitbox();
				add(characters);
		}
		...
		if (curStage == 'home'){
			var frontCharacters:BGSprite = new BGSprite('stageStuff/Background', -450, 400, ['front xd'], true);
			frontCharacters.scale.set(0.8, 0.8);
			frontCharacters.updateHitbox();
			add(frontCharacters);

			muerdago = new FlxSprite(-330, -10).loadGraphic(Paths.image('stageStuff/Intermedium'));
			muerdago.antialiasing = ClientPrefs.globalAntialiasing;
			muerdago.scale.set(0.8, 0.8);
			muerdago.updateHitbox();
			muerdago.alpha = 0;
			add(muerdago);
		}
		...
		add(healthBarBG);
		if(ClientPrefs.downScroll) healthBarBG.y = 30;

		healthBar = new FlxBar(341, 643, RIGHT_TO_LEFT, 588, 10, this,
			'health', 0, 2);
		healthBar.scrollFactor.set();
		// healthBar
		healthBar.visible = !ClientPrefs.hideHud;
		if(ClientPrefs.downScroll) healthBar.y = 83;
		add(healthBar);
		...
		scoreTxt = new FlxText(0, 680, FlxG.width, "", 20);
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.hideHud;
		if(ClientPrefs.downScroll) scoreTxt.y = 25;
		add(scoreTxt);
	...
	override public function update(elapsed:Float)
	{
		...
		if (curSong == 'Eggnog' && curBeat == 232)
		{
			FlxTween.tween(muerdago, {alpha: 1}, 1, {ease: FlxEase.circOut});
		}
			...
			case 'Pado-Beep':
				switch (curBeat){
					...
					case 128:
						...
						FlxTween.tween(muerdago, {alpha: 1}, 1, {ease: FlxEase.circOut});
					case 224:
						FlxTween.tween(muerdago, {alpha: 0}, 1, {ease: FlxEase.circOut});
					case 320:
						FlxTween.tween(muerdago, {alpha: 1}, 1, {ease: FlxEase.circOut});
					case 384:
						FlxTween.tween(muerdago, {alpha: 0}, 1, {ease: FlxEase.circOut});
	...
	override function beatHit()
	{
		...
		iconP1.setGraphicSize(Std.int(iconP1.width + 90));
		iconP2.setGraphicSize(Std.int(iconP2.width + 90));
